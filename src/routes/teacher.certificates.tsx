...
function TeacherCertificates() {
  const { user } = useAuth();
  const { courses, users, certificates, assessments, submissions, progress, requestCertificate } =
    useData();
    ...
    for (const c of myCourses) {
      ...
      for (const sid of c.studentIds) {
        const existing = certificates.find(
          (cert) => cert.studentId === sid && cert.courseId === c.id && cert.status !== "rejected",
        );
  ...
  const submit = () => {
    if (!studentId || !courseId) {
      toast.error("Pick a student and course.");
      return;
    }
  ...
  const handleVerify = (e: React.FormEvent) => {
    ...
    const cert = certificates.find(
      (c) => c.id.toLowerCase() === id.toLowerCase() && c.status === "approved",
    );
        ...
        {verifyResult &&
          (verifyResult.ok && verifyResult.cert ? (
            <div className="rounded-lg border border-success/40 bg-success/10 p-3 text-sm">
              ...
              <div className="mt-1">
                {studentName(verifyResult.cert.studentId)} —{" "}
                {courseName(verifyResult.cert.courseId)} · Issued{" "}
                {verifyResult.cert.issuedAt ?? "—"}
              </div>
            ...
            <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">
              Not found, not approved, or invalid ID.
            </div>
          ))}
      </GlassCard>
        ...
        <TabsContent value="eligible" className="mt-6">
          {eligible.length === 0 ? (
            <GlassCard className="text-center py-12 text-sm text-muted-foreground">
              No eligible students right now. Once learners enrol and progress through your courses
            ...
            <div className="space-y-4">
              ...
              {eligibleTotalPages > 1 && (
                <div className="flex items-center justify-between pt-3 border-t border-border/60 text-xs text-muted-foreground">
                  <span>
                    Showing {(currentEligiblePage - 1) * ITEMS_PER_PAGE + 1}–
                    {Math.min(currentEligiblePage * ITEMS_PER_PAGE, eligible.length)} of{" "}
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      variant="outline"
                      disabled={currentEligiblePage === 1}
                    ...
                    <span className="font-semibold text-foreground">
                      Page {currentEligiblePage} of {eligibleTotalPages}
                    </span>
                    <Button
                      size="sm"
                      variant="outline"
                      disabled={currentEligiblePage === eligibleTotalPages}
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              ...
                              className={`mt-1 inline-flex items-center gap-1 text-xs cursor-pointer ${sus > 0 ? "text-amber-500 font-semibold" : "text-muted-foreground"} hover:underline`}
                            >
                              <Eye className="h-3 w-3" /> Proctor log · {log.length} events
                              {sus > 0 ? ` · ${sus} flagged` : ""}
                            </button>
                          );
                        })()}
              ...
              {historyTotalPages > 1 && (
                <div className="flex items-center justify-between pt-3 border-t border-border/60 text-xs text-muted-foreground">
                  <span>
                    ...
                    {Math.min(currentHistoryPage * ITEMS_PER_PAGE, myCerts.length)} of{" "}
                    {myCerts.length} requests
                  </span>
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {viewingLog && (
                <>
                  {studentName(viewingLog.studentId)} · {courseName(viewingLog.courseId)} ·{" "}
                  {viewingLog.proctorLog?.length ?? 0} events ·{" "}
                  {viewingLog ? susCount(viewingLog) : 0} flagged
                </>
              )}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Admin approval required before the certificate is officially issued.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                placeholder="Why is this learner ready?"
                className="text-xs"
              />
            </div>
          </div>
            ...
            <div className="space-y-6 py-2">
              <div className="rounded-2xl border-2 border-red-500/40 bg-gradient-to-br from-card via-secondary/20 to-background p-6 sm:p-8 text-center space-y-4 shadow-xl relative overflow-hidden">
                ...
                <div className="text-xs text-muted-foreground">
                  has successfully completed the curriculum and examinations for
                </div>
                ...
                <div className="pt-3 text-[11px] font-mono text-muted-foreground flex items-center justify-center gap-2">
                  ...
                  <span>
                    Verified ID: <strong className="text-foreground">{previewCert.id}</strong>
                  </span>
                </div>
              </div>
