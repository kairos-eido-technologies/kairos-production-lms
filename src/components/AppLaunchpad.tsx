...
export function AppLaunchpad({ items }: AppLaunchpadProps) {
  ...
  const appModules = items.filter(
    (it) =>
      it.label !== "Dashboard" && it.label.toLowerCase().includes(filter.trim().toLowerCase()),
  );
        ...
        {appModules.length === 0 ? (
          ...
          <div className="flex flex-wrap items-center justify-center gap-y-12 gap-x-10 sm:gap-x-14 sm:gap-y-14 max-w-4xl px-2">
            {appModules.map((it) => {
              ...
              return (
                <div key={it.to} className="flex flex-col items-center px-1 py-1">
                  <Link
                    to={it.to}
                    preload="intent"
                  ...
                  >
                    ...
                    >
                      <Icon
                        className={`h-9 w-9 sm:h-10 sm:w-10 ${accent.icon} group-hover:scale-110 transition-transform duration-200 filter drop-shadow-sm`}
                      />
