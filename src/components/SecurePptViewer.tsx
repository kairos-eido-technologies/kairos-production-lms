...
export function SecurePptViewer({ url, title }: SecurePptViewerProps) {
  // ── No URL ────────────────────────────────────────────────────────────────
  if (!url || url.trim() === "") {
    return (
  ...
  if (cleanUrl.includes("docs.google.com/presentation")) {
    ...
    return (
      <div className="relative w-full rounded-xl border border-border overflow-hidden" style={{ height: 680 }}>
        <iframe src={embedUrl} className="w-full h-full border-0" title={title} allowFullScreen />
  ...
  // ── Render PowerPoint presentation using the exact same SecurePdfViewer engine ──
  return <SecurePdfViewer url={cleanUrl} title={title} isPresentation={true} />;
}
